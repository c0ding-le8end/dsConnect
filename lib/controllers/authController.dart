...
import 'package:ds_connect/util/Root.dart';
...
RxInt signUpPageSelected=RxInt(0);
...
Rx<FirebaseFirestore>? document;
  ...
  String? get user => firebaseUser.value?.uid;
  ...
  // int currentScreen=0;
  RxBool invalidName=RxBool(true),invalidUserName=RxBool(true),invalidEmail=RxBool(true),invalidPassword=RxBool(true);
...
RxBool safeExit=RxBool(false);
  ...
  onInit() async{
    ...
    await FirebaseFirestore.instance.collection("accounts").doc(firebaseUser.value!.uid).get().then((value) {
  ...
  void createUser(String name, String email, String password,String userName) async {
    try {
      FirebaseRepo firebaseRepo=Get.find<FirebaseRepo>();
       await _auth.createUserWithEmailAndPassword(
          email: email.trim(), password: password).then((value) {
            firebaseUser.value=value.user;
            firebaseUser.value!.updateDisplayName(userName);
            FirebaseFirestore.instance.collection("accounts").doc(firebaseUser.value!.uid).set({
              "name":name,
              "userName":userName,
              "email":email,
              ...
              this.email.value="";
              this.password.value="";
              this.userNameSetter.value="";
              this.profileNameSetter.value="";
            });
       });
      ...
      showDialog(context: context!, builder: (context)
      {
        ...
        return errorDialog("error creating email",e.message.toString());
      });
    }
  ...
  void login(String email, String password,BuildContext context) async {
    try {
      FirebaseRepo firebaseRepo=Get.find<FirebaseRepo>();
      print("the error seems to be here");
     await _auth.signInWithEmailAndPassword(
      ...
      if(e.message.toString()=="There is no user record corresponding to this identifier. The user may have been deleted.")
        message="The email entered is either invalid or does not exist";
  ...
  void signOut() async {
    signUpPageSelected.value=0;
    try {
      await _auth.signOut();
      Get.back();
      Get.offAll(Root(),transition: Transition.fade);
      passwordController.clear();
    } on FirebaseAuthException catch (e) {
      showDialog(context: context!, builder: (context)
      {
        return errorDialog("error Signing Out",e.message.toString());
      });
    }
  }
...
FirebaseFirestore.instance.collection("accounts").doc(firebaseUser.value!.uid).update({"profilePic":profilePic!.value});
    ...
    {
      showDialog(context: context!, builder: (context)=>AlertDialog(content: Text("${e.message.toString()}"),actions: [TextButton(onPressed: ()=>Get.back(), child: Text("Ok"))],));
      print(profilePic!.value);
    }
